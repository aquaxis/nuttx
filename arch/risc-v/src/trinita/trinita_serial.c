/****************************************************************************
 * arch/risc-v/src/trinita/trinita_serial.c
 *
...
#include "chip.h"
#include "trinita.h"
#include "trinita_config.h"
#include "trinita_lowputc.h"
#include "trinita_serial.h"
...
#  if defined(CONFIG_TRINITA_UART0)
#    define TTYS0_DEV       g_uart0port     /* UART0 is ttyS0 */
#    undef  TTYS1_DEV                       /* No ttyS1 */
#    define SERIAL_CONSOLE  1
#  else
...
/****************************************************************************
 * Name: trinita_uart_init
 ****************************************************************************/

static void trinita_uart_init(struct up_dev_s *priv)
{
  /* ToDo */
}
...
static int up_setup(struct uart_dev_s *dev)
{
  ...
  trinita_init_uart_pins(priv);

  trinita_uart_init(priv);
...
static int up_receive(struct uart_dev_s *dev, unsigned int *status)
{
  ...
  rxdata = getreg32(priv->uartbase + 0x04);
...
static void up_send(struct uart_dev_s *dev, int ch)
{
  ...
  putreg32((uint32_t)ch, priv->uartbase + 0x10);
}
